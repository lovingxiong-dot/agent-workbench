# -*- mode: python ; coding: utf-8 -*-
"""PyInstaller spec for AI Agent Workbench V5 (standalone)"""
import os
import PySide6

pyside6_dir = os.path.dirname(PySide6.__file__)

# Qt WebEngine 在打包时需要额外资源与辅助进程，否则 exe 内浏览器不可用
webengine_datas = [
    (os.path.join(pyside6_dir, 'QtWebEngineProcess.exe'), 'PySide6'),
    (os.path.join(pyside6_dir, 'resources'), 'PySide6/resources'),
    (os.path.join(pyside6_dir, 'translations', 'qtwebengine_locales'), 'PySide6/translations/qtwebengine_locales'),
]

a = Analysis(
    ['main.py'],
    pathex=[],
    binaries=[],
    datas=[
        ('config/config.yaml', 'config'),
        ('assets/app.ico', 'assets'),
        ('resources/themes/dark_github.qss', 'resources/themes'),
        ('resources/themes/trae_dark.qss', 'resources/themes'),
        (r'venv\Lib\site-packages\certifi\cacert.pem', 'certifi'),
    ] + webengine_datas,
    hiddenimports=[
        # Core infrastructure
        'yaml',
        'PySide6.QtSvg',
        'core.event_bus',
        'core.events',
        # V5 new UI architecture
        'v5.main',
        'v5.main_window',
        'v5.service.config_service',
        'v5.service.session_service',
        'v5.service.adapter',
        'v5.service.chat_service',
        'v5.controller.work_controller',
        'v5.model.events',
        'v5.widgets.base',
        'v5.widgets.window_frame',
        'v5.widgets.left_panel',
        'v5.widgets.chat_items',
        'v5.widgets.chat_scene',
        'v5.widgets.chat_area',
        'v5.widgets.right_panel',
        'v5.widgets.terminal_widget',
        'v5.widgets.file_reader_widget',
        'v5.widgets.browser_widget',
        'v5.widgets.settings_dialog',
        'v5.widgets.dropdown_selector',
        'PySide6.QtWebEngineWidgets',
        'PySide6.QtWebEngineCore',
        'PySide6.QtWebChannel',
        'PySide6.QtWebSockets',
        'PySide6.QtSql',
        # agent_engine engines
        'agent_engine',
        'agent_engine.llm_registry',
        'agent_engine.memory_manager',
        'agent_engine.phase_manager',
        'agent_engine.orchestrator',
        'agent_engine.agent_session',
        'agent_engine.engines',
        'agent_engine.engines.interfaces',
        'agent_engine.engines.context_engine',
        'agent_engine.engines.prompt_engine',
        'agent_engine.engines.inference_engine',
        'agent_engine.engines.tool_engine',
        'agent_engine.engines.phase_engine',
        'agent_engine.engines.memory_engine',
        'agent_engine.engines.metrics_engine',
        'agent_engine.engines.policy_engine',
        # tools
        'tools',
        'tools.system',
        'tools.quant',
        'tools.mt5',
        'tools.external_apis',
        'tools.screen',
        # workers
        'workers',
        'workers.agent_worker',
        'workers.base_worker',
        'workers.terminal_worker',
        'workers.session_task',
        'workers.task_capacity',
        'workers.task_queue',
        'workers.worker_pool',
        # services still referenced by tests / fallback paths
        'services',
        'services.pending_queue',
        'services.task_service',
        'services.config_service',
        'services.session_service',
        'services.theme_service',
        'services.project_service',
        'services.activity_service',
        'services.context_service',
        'services.path_resolver',
        'services.metrics_collector',
        'services.interpreter_service',
        'services.persistence_service',
        'services.mcp_service',
        # markdown & web parsing
        'markdown',
        'markdown.extensions.fenced_code',
        'markdown.extensions.tables',
        'markdown.extensions.nl2br',
        'markdown.extensions.codehilite',
        'bs4',
        'lxml',
        'html5lib',
        # others
        'dotenv',
        'sqlite3',
        'certifi',
    ],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=['scripts/runtime_hook.py'],
    excludes=[
        'PySide6.QtQml', 'PySide6.QtQuick',
        'PySide6.QtMultimedia', 'PySide6.QtMultimediaWidgets',
        'PySide6.QtBluetooth', 'PySide6.QtNfc',
        'PySide6.QtSensors', 'PySide6.QtSerialPort',
        'PySide6.QtPositioning', 'PySide6.QtLocation',
        'PySide6.QtTextToSpeech', 'PySide6.QtHelp',
        'PySide6.QtAxContainer', 'PySide6.QtQuickWidgets',
        'PySide6.QtQuickControls2', 'PySide6.QtDataVisualization',
        'setuptools', 'pip', 'wheel', 'pkg_resources',
    ],
    noarchive=False,
    optimize=0,
)
pyz = PYZ(a.pure)

exe = EXE(
    pyz,
    a.scripts,
    [],
    exclude_binaries=True,
    name='AgentWorkbench',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    console=False,
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    icon=['assets/app.ico'],
)
coll = COLLECT(
    exe,
    a.binaries,
    a.datas,
    strip=False,
    upx=True,
    upx_exclude=[],
    name='AgentWorkbench',
)
