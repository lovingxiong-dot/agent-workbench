...
├── experiments/            # [ui-template] UI 原型实验区（独立于主项目）
│   ├── ui_template.py      #   独立 PySide6 UI 原型（~2100行）
│   └── ui_template_with_handle.py  #   含 resize handle 变体
├── docs/                   # 开发者文档
│   ├── getting-started.md  # 5 分钟上手指南
│   └── ui/效果图/          # [ui-template] SVG/PNG UI 设计稿效果图
...
| v0.4-alpha | 2026-07-02 | [ui-template] 根目录净化：恢复核心文档（README/ARCHITECTURE/PROJECT_BLUEPRINT）、UI 原型归位 experiments/、临时文件清理 | refactor | README.md, ARCHITECTURE.md, PROJECT_BLUEPRINT.md, experiments/*, scripts/*, docs/ui/效果图/* |
| v0.1 | 2026-07-01 | [ui-template] UI 模板图标 SVG 化 + 标签背景框恢复 + 输入区拖拽 + 防抖 + 边缘 resize 分析 | feat/fix | ui_template.py, ui_template_with_handle.py |
