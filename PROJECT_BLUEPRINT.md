...
# Project Blueprint
## 元信息
| 项目名称 | AI Agent 工作台 | 当前版本 | v0.2 | 存档次数 | 2 |
...
| 本地模型 | Ollama (OpenAI compatible) | - | 本地 qwen3:4b / gemma2:2b 推理 |
...
## 目录结构
/
├── agent_engine/       # 模式管理、LLM 注册（含CRUD）、记忆管理
├── tools/              # 系统命令、量化、MT5 等工具实现
├── ui/                 # UI 组件（当前由 main.py 统一维护）
├── storage/            # 运行时数据（SQLite / JSON / 向量存储）
├── tests/              # 测试目录
├── main.py             # 主程序入口（含 SettingsDialog、ProviderFormDialog）
├── config.yaml         # 配置驱动文件
...
## 最近变更
| 版本 v0.2 | 日期 2026-06-25 | 描述 DeepSeek密钥修复与模型选择下拉功能 | 类型 feat/fix | 文件 config.yaml, llm_registry.py, main.py |
