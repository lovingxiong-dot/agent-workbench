...
> **The active development line is `v6-dev` at version `v6.7.0-alpha`.**
> **Public baseline: `v6.0.0-alpha`.**

| Item | Value |
|---|---|
| Active branch | `v6-dev` |
| Current development version | `v6.7.0-alpha` |
| Public baseline | `v6.0.0-alpha` |
...
## Mission
完成 V6 Step 5.4：建立 Runtime Orchestration Foundation。新增 `Orchestrator` 层与 Task Lifecycle State Machine（CREATED → PLANNING → EXECUTING → COMPLETED/FAILED），让 AgentRuntime 从"调用者驱动 execute"升级为"Runtime 接收 Task 后自主推进生命周期"。当前不接真实 LLM，不做自治循环。
...
- `EngineManager` 负责 Engine 生命周期；`EventBus` 负责运行时通信；`CapabilityRegistry` 负责 Engine 选择；`ReplayService` 负责执行轨迹记录与回放基础；`Orchestrator` 负责任务编排与生命周期；五者职责分离。
- `ReplayService` 不是 Engine，不替代 `EngineManager` 或 `EventBus`。
- `Planner` 不是 Runtime 大脑；`Orchestrator` 才是编排入口，Planner 只是其可调度的决策能力之一。
- 调用者不再硬编码 Engine 名称，而是描述能力需求。
- 第一版 Replay 只做轨迹记录、导出、查看，不重新执行 LLM/Tool/Memory。
- 第一版 Orchestrator 只做线性 Task Lifecycle + EventBus 驱动，不做 LLM 决策与自治循环。
...
- [x] Step 5.4 完成：新增 `Orchestrator` 与 Task Lifecycle State Machine，AgentRuntime 持有 Orchestrator；标签 `v6.7.0-alpha`。
- [ ] Step 5.5 / V6.8：Planner Decision Loop（基于 Orchestrator 的事件驱动决策循环，仍不接真实 LLM）。

## Step 5.4 Details
- [x] 扩展 `v6/runtime/enums.py`：`RuntimeState` 新增 `PLANNING` / `EXECUTING`。
- [x] 扩展 `v6/runtime/state_machine.py`：支持 `CREATED -> PLANNING -> EXECUTING -> COMPLETED/FAILED`。
- [x] 新增 `v6/runtime/orchestrator.py`：
  - `Orchestrator` 维护 Task 生命周期状态。
  - 订阅/发布 EventBus 事件驱动流程。
  - 使用 CapabilityRegistry 选择 Engine（非硬编码名称）。
  - 不直接访问 Trace。
- [x] 升级 `v6/runtime/runtime.py`：
  - `AgentRuntime` 默认创建并持有 `Orchestrator`。
  - 新增 `orchestrator` 属性与 `orchestrate(task)` 方法。
- [x] 新增 `tests/v6/test_v6_orchestrator.py` 共 7 个测试。
- [x] V6 全量测试 `pytest tests/v6/` **166/166 通过**。
...
## Key Files
- `v6/runtime/orchestrator.py` — Runtime Orchestration Foundation（Task Lifecycle + EventBus 驱动）。
- `v6/runtime/runtime.py` — AgentRuntime（持有 Orchestrator / EngineManager / EventBus）。
...
- venv: none
- last_commit: a1c44f0 feat(runtime): Step 5.3 Runtime Trace Replay Foundation [test:159/159] [hint:v6,runtime,replay,step5.3] (by AI-Kimi-K2.7-Code)
...
1. **Step 5.5 / V6.8：Planner Decision Loop**（当前最高优先级）
   - 在 `Orchestrator` 基础上引入事件驱动的决策循环（仍不接真实 LLM）。
   - 目标：Planner 作为 Orchestrator 的决策能力之一，通过 Capability Query + EventBus 驱动多 Engine 协作。
   - 循环骨架：Observe → Reason → Plan → Act → Evaluate → Repeat（第一阶段用规则/占位实现 Reason/Evaluate）。
2. **Runtime Task 模型完善**
...
4. **暂不实现八大 Engine 真实业务逻辑**
   - LLM/Tool/Memory 等功能开发应在 Runtime 基础（Event Bus + Capability Registry + Replay + Orchestration + Task Model）稳固后再进行。
