...
class LeftPanel(QWidget):
    ...
    def load_sessions(self, sessions: list) -> None:
        """从 SessionViewModel 列表注入会话数据。
        
        不修改任何视觉属性，仅将 ViewModel 转换为 SessionGroup 格式。
        """
        data: list[tuple[str, str, list[dict]]] = []
        for s in sessions:
            item = {
                "id": s.id,
                "title": s.title,
            ...
            for g in data:
                if g[0] == gid:
                    ...
                    break
            if not found:
                data.append((gid, gid, [item]))
        self.update_sessions(data)

    def load_agents(self, agents: list) -> None:
        """从 AgentViewModel 列表注入 Agent 数据到会话列表。
        
        将 Agent 列表作为特殊分组展示在会话列表中。
        不修改任何视觉属性。
        """
        items: list[dict] = []
        for a in agents:
            items.append({
                "id": a.id,
                "title": a.name,
                "preview": a.description or a.role or "",
                "timestamp": a.status or "",
            })
        if items:
            self.update_sessions([("agents", "Agents", items)])
